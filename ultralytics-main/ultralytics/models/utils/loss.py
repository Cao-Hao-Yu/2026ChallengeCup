...
class DETRLoss(nn.Module):
    ...
    def __init__(
        ...
        if loss_gain is None:
            loss_gain = {"class": 1, "bbox": 5, "giou": 2, "no_object": 0.1, "mask": 1, "dice": 1, "base_class": 0.1}
        self.nc = nc
        self.nbc = nbc
    ...
    def _split_hierarchical_class(self, raw_cls: torch.Tensor):
    ...
    def _get_loss_class_hierarchical(
        self,
        ...
        valid_mask = targets != self.nc
        target_spec_one_hot = torch.zeros((bs, nq, self.nc), device=device, dtype=torch.float)
        target_base_one_hot = torch.zeros((bs, nq, self.nbc), device=device, dtype=torch.float)
        spec_supervise_mask = torch.zeros((bs, nq), device=device, dtype=torch.bool)

        loss_cls = pred_scores.new_zeros(())
        loss_base_cls = pred_base_scores.new_zeros(())
        norm_factor = max(num_gts, 1) / nq

        if valid_mask.any():
            valid_b, valid_n = torch.where(valid_mask)
            raw_labels = targets[valid_mask]
            gt_base, gt_spec = self._split_hierarchical_class(raw_labels)

            target_base_one_hot[valid_b, valid_n, gt_base - 1] = 1.0
            has_spec = gt_spec != 25
            spec_b = valid_b[has_spec]
            spec_n = valid_n[has_spec]
            spec_c = gt_spec[has_spec]

            target_spec_one_hot[spec_b, spec_n, spec_c] = 1.0
            spec_supervise_mask[spec_b, spec_n] = True

            spec_loss_mat = F.binary_cross_entropy_with_logits(
                pred_scores, target_spec_one_hot, reduction="none"
            )
            spec_loss_mat = spec_loss_mat * spec_supervise_mask.unsqueeze(-1)
            loss_cls = spec_loss_mat.mean(1).sum() / norm_factor

            base_loss_mat = F.binary_cross_entropy_with_logits(
                pred_base_scores, target_base_one_hot, reduction="none"
            )
            loss_base_cls = base_loss_mat.mean(1).sum() / norm_factor

        return {
            f"loss_class{postfix}": loss_cls * self.loss_gain["class"],
            f"loss_base_class{postfix}": loss_base_cls * self.loss_gain.get("base_class", 1.0),
        }
    ...
    def _get_loss_aux(
        ...
        for i, (aux_bboxes, aux_scores, aux_base_scores) in enumerate(zip(pred_bboxes, pred_scores, pred_base_scores)):
            layer_loss = self._get_loss(
                aux_bboxes, aux_scores, aux_base_scores, 
                gt_bboxes, gt_cls, gt_groups, 
                postfix=f"{postfix}_aux{i}",
    ...
    def _get_loss(
        ...
        pred_bboxes_matched = pred_bboxes[idx]     # (K, 4)
        gt_bboxes_matched = gt_bboxes[gt_idx]

        bs, nq = pred_scores.shape[:2]
        targets = torch.full((bs, nq), self.nc, device=pred_scores.device, dtype=gt_cls.dtype)
        targets[idx] = gt_cls[gt_idx]

        losses = self._get_loss_class_hierarchical(
            pred_scores, pred_base_scores, targets, len(gt_bboxes_matched), postfix
        )
        losses.update(self._get_loss_bbox(pred_bboxes_matched, gt_bboxes_matched, postfix))
        return losses
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        pred_bboxes, pred_scores, _, _, _, pred_base_scores, _ = preds
...
class RTDETRDetectionLoss(DETRLoss):
    def forward(
        ...
        total_loss = self._get_loss_main(
            pred_bboxes, pred_scores, pred_base_scores, 
            gt_bboxes, gt_cls, gt_groups
        )
        ...
        if dn_meta is not None:
            ...
            # 如果 dn_base_scores 为 None，移除可能产生的空损失项
            dn_loss = {k: v for k, v in dn_loss.items() if v is not None}
            total_loss.update(dn_loss)
            
        else:
            # 若无 DN 数据，损失置零
            loss_keys = ["loss_bbox", "loss_giou", "loss_class", "loss_base_class"]
            total_loss.update({f"{k}_dn": torch.tensor(0.0, device=self.device) for k in loss_keys})

        return total_loss

    def _get_loss_main(self, pred_bboxes, pred_scores, pred_base_scores, gt_bboxes, gt_cls, gt_groups):
        total_loss = self._get_loss(
            pred_bboxes[-1], 
            pred_scores[-1], 
            pred_base_scores[-1], 
